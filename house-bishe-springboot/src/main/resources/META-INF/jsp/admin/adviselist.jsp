...
<html>
	...
	$(document).ready(function(){
		$.post("<%=basePath%>adviseList",function(data){//获取json数据
		var data = eval("("+data+")");
		tableInit(data);
	});
    ...
    <script>
                   ...
                   field: 'createtime',
                	...
                	formatter: function (value, row, index) {
                        ...
                        'click .getDetail': function (e, value, row, index) {
                           detail(row.advise_id);//调用details(sno)
                        }
                    }
                }
        ...
        $(function () {
            $button.click(function () {//获取删除的多选框里里面的值,将值传给:url:"deleteMembers",
                var advise_ids = $.map($table.bootstrapTable('getSelections'), function (row) {
                    return row.advise_id;
                });
                var advise_idsStr = advise_ids.toString();
                $.ajax({
                	url:"<%=basePath%>adviseDeletes",
                	data:{
                		idsStr:advise_idsStr//servlet获取的参数
                	},
