...
<html>
	...
	$(document).ready(function(){//记住这里有点乱在customerController
		$.post("<%=basePath%>customer/commentlist",function(data){//获取json数据
		var data = eval("("+data+")");
		tableInit(data);
	});
            ...
            $button.click(function () {//获取删除的多选框里里面的值,将值传给:url:"deleteMembers",
                ...
                $.ajax({
                	...
                	error:function(result) {//假若批量删除失败
                		//alert(result);
                		$.post("<%=basePath%>customer/commentlist",function(data){
            				var data = eval("("+data+")");
            				tableInit(data);
            			});
                	}
