...
<table class="table table-hover table-outline mb-0 hidden-sm-down">
                                        <thead class="thead-default">
                                            <tr>
                                                ...
                                                <th class="icon-graduation" style="font-size: 20px"></th>
										 ...
										 <c:forEach var="c" items="${clist}" varStatus="status">
                                            <tr>
                                                <td class="text-center">
                                                    <div class="avatar">
                                                        <img src="/head/${c.img }" class="img-avatar" alt="admin@bootstrapmaster.com">
                                                ...
                                                <td>
                                               <span class="mar">${c.job }</span> 
                                                </td>
												
												
                                                <td class="text-center">
                                                 <!--    <i class="fa fa-cc-mastercard" style="font-size:24px"><a href="">回复</a></i> -->
                                                <a  onclick="addNotice('${c.customer_id}','${c.nickname}','${c.img}')"  href="#myModal" data-toggle="modal" >发送留言</a>
                                                </td>
                                                <td>
                                                    <div class="small text-muted">Last login</div>
                                                  <span class="mar2">  <strong> ${c.createtime }</strong></span> 
                                                </td>
                                                <td>
                                                <span class="mar3">    ${c.mobile }</span>
                                                </td>
                           </tr>
...
<input type="hidden" class="form-control" id="customer_id" name="customer_id"  ></input>
...
addNotice=function(customer_id,nickname,img){
...
reply=function(){
  var rece_id=$("#customer_id").val();
  var nickname=$("#nickname").val();
  var img=$("#img").val();
  var content=$("#content").val();
   $.ajax({
    url:"<%=basePath%>notice/addNotice_p",
    data:{
    	rece_id:rece_id,
    	content:content,
    	nickname:nickname,
